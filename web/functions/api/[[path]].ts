// Cloudflare Pages Function — GTMC Scanner API
// src/db.ts
...
// Inline CSV sync parser (no Buffer dependency)
function parseCsvSync(text, opts = {}) {
  const delim = opts.delimiter || (text.includes("\\t") && !text.includes(",") ? "\\t" : ",");
  return text.split("\\n").filter(l => l.trim().length > 0).map(l => l.split(delim).map(cell => cell.trim()));
}
...
app.post("/api/admin/box-types/import", jwtMiddleware, requireRole("admin"), async (c) => {
  ...
  if (!file) throw new HTTPException(400, { message: "请上传CSV文件" });
  const content = await file.text();
  const records: string[][] = parseCsvSync(content, { skip_empty_lines: true });
  const results = { created: 0, skipped: 0, errors: [] as string[] };
...
app.get("/api/admin/monitor/audit-logs", jwtMiddleware, requireRole("admin"), async (c) => {
  ...
  const [{ count }] = await db`SELECT COUNT(*) as count FROM audit_logs WHERE action = 'RESET_SCAN_COUNT'`;
  const items = await db`SELECT * FROM audit_logs WHERE action = 'RESET_SCAN_COUNT' ORDER BY created_at DESC LIMIT ${pageSize} OFFSET ${offset}`;
  return c.json({ list: items, total: parseInt(count), page, pageSize });
});
